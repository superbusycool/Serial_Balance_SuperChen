...
function [K,Q,R] = get_k_length_theta_offset(leg_length)
    ...
    R1=0.077;                          %驱动轮半径
    l1= 0.05423;                       %机体质心距离转轴距离; 
    mw1=0.322;                        %驱动轮质量
    mp1=2.751;                        %杆质量
    M1=8.4;                      %机体质量
    Iw1=mw1*R1^2;                     %驱动轮转动惯量
    IM1=0.143;       %机体绕质心转动惯量,机体的长宽
    theta_offset=0.1;     %机体质心与同一竖直切面和机体转轴在此竖直平面投影点连线,与z轴的夹角角度(rad)
    ...
    % 索引合法性检查：确保索引在1~20范围内
    if idx < 1 || idx > size(Leg_data,1)
        error(['腿长 %f 对应的索引 %d 越界！Leg_data仅包含0.12~0.31（步长0.01）的参数',...
    ...
    Q=diag([12 50 1 1 500 10]);
    R=diag([2.75 0.5]);

    %起立
   % Q=diag([10 10 1 1 500 1]);
   %  R=diag([2.75 1.25]);
