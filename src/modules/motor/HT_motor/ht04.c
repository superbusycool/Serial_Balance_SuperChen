...
ht_motor_object_t *ht_motor_register(motor_config_t *config, void *control)
{
    ...
    {
    case 1:
        ht_motor_obj[idx].can = &hfdcan1;
        break;
    case 2:
        ht_motor_obj[idx].can = &hfdcan2;
        break;
    default:
        break;
    }
...
// static float ht_send_t[4];
// float ht_obs[4];
//
///*目前是以护栏较窄的一侧为正方向,从正方向向后看去,right_front:id 2 ; right_back:id 3;left_front:id 1;left_back:id 4*/
///* 1 号电机 */
...
//
//
//     if(chassis_cmd.ctrl_mode == CHASSIS_INIT && chassis_fdb_data.leg_state == LEG_BACK_STEP)   /*TODO 按照目前的电机位置来重新整定力矩正负号,报错vmc部分的正负号*/
//     {
//         ht_send_t[0] = -HT_INIT_OUT;
////        ht_send_t[0] = 0 ;
//     }
//     else
//     {
//
//         ht_send_t[0] = vmc_out_l[1] * HT_RATIO;
//         LIMIT_MIN_MAX(ht_send_t[0], -HT_OUTPUT_LIMIT, HT_OUTPUT_LIMIT);
//         ht_obs[0] = ht_send_t[0] ;
//
//
//#ifdef HT_CLOSE_ALL_MOTOR
//         ht_send_t[0] = 0;
//#endif
...
//
//
//     if(chassis_cmd.ctrl_mode == CHASSIS_INIT && chassis_fdb_data.leg_state == LEG_BACK_STEP)
//     {
//         ht_send_t[1] = HT_INIT_OUT;
//
//     }
//     else
//     {
//
//         ht_send_t[1] = -vmc_out_r[1] * HT_RATIO;
//         LIMIT_MIN_MAX(ht_send_t[1], -HT_OUTPUT_LIMIT, HT_OUTPUT_LIMIT);
//         ht_obs[1] = ht_send_t[1] ;
//
//
//#ifdef HT_CLOSE_ALL_MOTOR
//         ht_send_t[1] = 0;
...
//     {
//
//         ht_send_t[2] = -vmc_out_r[0] * HT_RATIO ;
//         LIMIT_MIN_MAX(ht_send_t[2], -HT_OUTPUT_LIMIT, HT_OUTPUT_LIMIT);
//         ht_obs[2] = ht_send_t[2] ;
//
//
//#ifdef HT_CLOSE_ALL_MOTOR
//         ht_send_t[2] = 0;
...
//     {
//
//         ht_send_t[3] = vmc_out_l[0] * HT_RATIO;
//         LIMIT_MIN_MAX(ht_send_t[3], -HT_OUTPUT_LIMIT, HT_OUTPUT_LIMIT);
//         ht_obs[3] = ht_send_t[3];
//
//
//#ifdef HT_CLOSE_ALL_MOTOR
//         ht_send_t[3] = 0;
...
//
//     /*电机id等可参考平步实物进行修改*/
//     motor_config_t ht_motor_config1 = {   //left_front
//             .motor_type = HT04,
//             .can_id = 2,
//             .tx_id = 1,
//             .rx_id = 0x10,
//     };
//     ht_motor[0] = ht_motor_register(&ht_motor_config1, ht_control[0]);
//
//     motor_config_t ht_motor_config4 = {     //left_back
//             .motor_type = HT04,
//             .can_id = 2,
//             .tx_id = 4,
//             .rx_id = 0x10,
//     };
//     ht_motor[3] = ht_motor_register(&ht_motor_config4, ht_control[3]);
//
//     motor_config_t ht_motor_config2 = {     //right_front
//             .motor_type = HT04,
//             .can_id = 2,
//             .tx_id = 2,
//             .rx_id = 0x10,
//     };
//     ht_motor[1] = ht_motor_register(&ht_motor_config2, ht_control[1]);
//
//     motor_config_t ht_motor_config3 = {     //roght_back
//             .motor_type = HT04,
//             .can_id = 2,
//             .tx_id = 3,
//             .rx_id = 0x10,
//     };
//     ht_motor[2] = ht_motor_register(&ht_motor_config3, ht_control[2]);
//
//
// }
