...
R_l_ac = 0.214;                            % 两个驱动轮之间距离/2         （单位：m）
l_c_ac = 0.05423;                            % 机体质心到腿部关节中心点距离  （单位：m）
m_w_ac = 0.577; m_l_ac = 0.804; m_b_ac = 6.120;    % 驱动轮质量 腿部质量 机体质量  （单位：kg）
I_w_ac = (970.514)*10^(-6);                            % 驱动轮转动惯量               （单位：kg m^2）
I_b_ac = 0.088434;                            % 机体转动惯量(自然坐标系法向)  （单位：kg m^2）
I_z_ac = 0.07564;                            % 机器人z轴转动惯量            （单位：kg m^2）
...
l_l_ac = 0.13;        % 左腿摆杆长度                      （左腿对应数据）  （单位：m）
l_wl_ac = 0.0158;       % 左驱动轮质心到左腿摆杆质心距离                      （单位：m）
l_bl_ac = 0.0110;       % 机体转轴到左腿摆杆质心距离                          （单位：m）
I_ll_ac = 0.01798949;      % 左腿摆杆转动惯量                                   （单位：kg m^2）
l_r_ac = 0.13;        % 右腿摆杆长度                      （右腿对应数据）  （单位：m）
l_wr_ac = 0.0158;       % 右驱动轮质心到右腿摆杆质心距离                      （单位：m）
l_br_ac = 0.0110;       % 机体转轴到右腿摆杆质心距离                          （单位：m）
I_lr_ac = 0.01798949;      % 右腿摆杆转动惯量                                   （单位：kg m^2）
...
% 第二列为l_wl，单位：m % 驱动轮质心到左腿摆杆质心距离
% 第三列为l_bl，单位：m % 机体转轴到左腿摆杆质心距离
% 第四列为I_ll，单位：kg m^2  %摆杆转动惯量
% （注意单位别搞错！）
% 行数根据L_0范围区间（，单位cm时）的整数数量进行调整

Leg_data_l = [0.12,  0.0156,  0.0108,  0.01769790; 
              0.13,  0.0158,  0.0110,  0.01798949;
              0.14,  0.0160,  0.0111,  0.01830103;
              0.15,  0.0162,  0.0112,  0.01858948;
              0.16,  0.0164,  0.0114,  0.01893331;
              0.17,  0.0166,  0.0115,  0.01930053;
              0.18,  0.0169,  0.0117,  0.01969116;
              0.19,  0.0172,  0.0119,  0.02010521;
              0.20,  0.0174,  0.0121,  0.02054272;
              0.21,  0.0177,  0.0123,  0.02100371;
              0.22,  0.0180,  0.0126,  0.02148821;
              0.23,  0.0183,  0.0127,  0.02199626;
              0.24,  0.0186,  0.0130,  0.02252789;
              0.25,  0.0190,  0.0132,  0.02308315;
              0.26,  0.0193,  0.0134,  0.02366207;
              0.27,  0.0196,  0.0137,  0.02426472;
              0.28,  0.0200,  0.0139,  0.02489113;
              0.29,  0.0203,  0.0142,  0.02554137;
              0.30,  0.0207,  0.0144,  0.02621551;
              0.31,  0.0210,  0.0147,  0.02691362]; %
% 以上数据应通过实际测量或sw图纸获得
...
for mat_row = 1:40  % 遍历K_Fit_Coefficients的40行
    % 计算当前行对应的控制行号（1-4）和状态列号（1-10）
    ctrl_row = ceil(mat_row / 10);  % 每10行对应1个控制行（10行→1，20行→2，…，40行→4）
    state_col = mat_row - (ctrl_row - 1) * 10;  % 每个控制行内的状态列（1-10）

    % 提取当前行的6个系数
    coeffs = K_Fit_Coefficients(mat_row, :);

    % 生成数组名（如a11, a12, ..., a410）
    arr_name = sprintf('a%d%d', ctrl_row, state_col);

    % 格式化输出（保留5位有效数字，与原始数据精度一致，无多余空格）
    fprintf('float %s[6] = {%.5g,%.5g,%.5g,%.5g,%.5g,%.5g};\n', ...
        arr_name, coeffs(1), coeffs(2), coeffs(3), coeffs(4), coeffs(5), coeffs(6));

    % 每10行（1个控制行）结束后空1行，增强可读性
    if mod(mat_row, 10) == 0
        fprintf('\n');
    end
end
